...
class LiquiditySnapshotService:
    ...
    def run(self) -> int:
        ...
        with session_scope() as session:
            ...
            if not positions:
                log.warning("liquidity_net_position returned no rows — skipping snapshot")
                return 0
        ...
        total_usd = sum(
            (r["liquidity_balance_usd"] for r in rows if r["liquidity_balance_usd"] is not None),
            Decimal(0),
        )
        log.info("Snapshot wrote %d currency rows; total net liquidity ≈ $%s", inserted, total_usd)
        return inserted
